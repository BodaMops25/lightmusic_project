...
#define BTN_PIN 2          // кнопка переключения режимов (PIN --- КНОПКА --- GND)
...
#define BRIGHTNESS_2_MIN 150 // наименьшая яркость для 2 режима
...
void loop() {
  ...
  if (millis() - hue_timer > RAINBOW_SPEED) {
    ...
    // пульсирующее изменение яркости
    // if(bright <= BRIGHTNESS_2_MIN) brightDirection = 0;
    // else if(255 <= bright) brightDirection = 1;
    // if(brightDirection == 0) bright++;
    // else if(brightDirection == 1) bright--;
...
void animation() {
  ...
  switch (this_mode) {
    ...
    case 2:
      ...
      // Llenght - значение звука
      
      // count = 0;
      for (int i = (MAX_CH - 1); i > ((MAX_CH - 1) - Rlenght); i--) {
        leds[i] = CHSV(color_red, 255, bright);
        // count++;
      }
      for (int i = (MAX_CH); i < (MAX_CH + Llenght); i++ ) {
        leds[i] = CHSV(color_blue, 255, bright);
      }
      break;
    case 3:
